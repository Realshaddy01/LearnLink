...
export const getProduct=(page,limit,search,order)=>(dispatch)=>{
    dispatch({type:PRODUCT_REQUEST})
    axios.get(`http://localhost:5000/courses?page=${page}&limit=${limit}&q=${search}&sortBy=price&sortOrder=${order}`,{
      headers:{
        Authorization:`Bearer ${token}`
      }
...
export const getUser=(page,limit,search,order)=>(dispatch)=>{
    dispatch({type:PRODUCT_REQUEST})
    axios.get(`http://localhost:5000/users?page=${page}&limit=${limit}`,{
      headers:{
        Authorization:`Bearer ${token}`
      }
...
export const getvideo=(page,limit)=>(dispatch)=>{
    dispatch({type:PRODUCT_REQUEST})
    axios.get(`http://localhost:5000/videos?page=${page}&limit=${limit}`,{
      headers:{
        Authorization:`Bearer ${token}`
      }
...
export const patchProduct=(id,data)=>(dispatch)=>{
  dispatch({type:PRODUCT_REQUEST})
  fetch(`http://localhost:5000/courses/update/${id}`,{
    method:"PATCH",
    headers:{
      "Content-Type":"application/json",
      Authorization:`Bearer ${token}`
...
export const deleteProduct=(id)=>(dispatch)=>{
    dispatch({type:PRODUCT_REQUEST});
    axios.delete(`http://localhost:5000/courses/delete/${id}`,{
      headers:{
        Authorization:`Bearer ${token}`
      }
